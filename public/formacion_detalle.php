<?php
// --------------------------------------------------------------
// formacion_detalle.php - Centro de Excelencia Operacional (CEO)
...
<head>
<meta charset="utf-8">
<title>Formaciones - Detalle Formacion #<?= htmlspecialchars($sol['nsolicitud']) ?> - <?= APP_NAME ?></title>
...
<div class="container mb-5">
  ...
  <h5 class="text-primary mb-3">
    ...
    Detalle Formacion N° <?= htmlspecialchars($sol['nsolicitud']) ?>
  </h5>
...
<?php if ($puedeCerrar && !$bloquearCerrar): ?>
  <div class="text-end mb-3" id="boxCerrarSolicitud" style="display:none;">
      <button class="btn btn-danger btn-sm" id="btnCerrarSolicitud">
          <i class="bi bi-lock-fill me-1"></i> Cerrar Formacion
      </button>
  </div>
<?php endif; ?>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.querySelectorAll('.ver-historial').forEach(btn => {
        btn.addEventListener('click', async e => {
            ...
            try {
                const resp = await fetch('formacion_historial.php?rut=' + encodeURIComponent(rut));
                const html = await resp.text();

                document.getElementById('historialContenido').innerHTML = html;

                document.getElementById('btnExportExcel').href =
                    'formacion_historial_excel.php?rut=' + encodeURIComponent(rut);
...
document.getElementById('btnCerrarSolicitud')?.addEventListener('click', async function() {
    ...
    await fetch('cerrar_formacion.php', {
        method: 'POST',
        headers: {'Content-Type':'application/x-www-form-urlencoded'},
        body: new URLSearchParams({ nsol: <?= (int)$sol['nsolicitud'] ?> })
    });

    alert("Formacion cerrada correctamente.");
    location.reload();
});
    ...
    if (!confirm('¿Deseas autorizar esta solicitud?')) return;
            ...
            setTimeout(() => {
              window.location.href =
                'formacion_autoriza_envio.php?id=<?= (int)$sol['nsolicitud'] ?>';
            }, 50);
});
</script>
