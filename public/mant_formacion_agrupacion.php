<?php
// /public/mant_formacion_agrupacion.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // EDITAR
    ...
    $stmt = $pdo->prepare("
      UPDATE ceo_formacion_agrupacion
        SET titulo = :titulo,
  ...
  // ELIMINAR
    ...
    $stmt = $pdo->prepare("DELETE FROM ceo_formacion_agrupacion WHERE id = :id");
    $stmt->execute([':id' => $id]);
...
$agrupaciones = $pdo->query("
  ...
  LEFT JOIN ceo_formacion_servicios s ON s.id = a.id_servicio
  ORDER BY a.id ASC
...
<head>
<meta charset="utf-8">
<title><?= APP_NAME ?> | Mantenimiento Agrupacion Formaciones</title>
