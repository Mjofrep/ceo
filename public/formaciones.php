...
// ----------------------------------------------------------
// Pagina de gestion de Formaciones del CEO (version final)
// ----------------------------------------------------------
...
// --- Caso 1: Admin o rol 5 y empresa ENEL ---
...
// --- Caso 3: Resto de roles ---
} else {

    // Puede ver lo de su empresa O lo que él solicitó
    $where = "(s.contratista = :empresa OR s.solicitante = :iduser)";
    $params[':empresa'] = $idEmpresa;
    $params[':iduser']  = $idUsuario;
}
...
try {
  $sql = "
    ...
    LEFT JOIN ceo_formaciontipo ch ON s.habilitacionceo = ch.id
    WHERE $where
...
<head>
  <meta charset="utf-8">
  <title>Formaciones - Gestion - <?= esc(APP_NAME) ?></title>
  ...
  <div class="card rounded-4 mb-4">
    <div class="card-body py-3 d-flex justify-content-between align-items-center flex-wrap gap-3">
      <div class="d-flex gap-3 align-items-center flex-wrap icon-btn">
        <i class="bi bi-plus-circle text-primary fs-4" title="Ingresar Formacion" id="btnNuevaSolicitud"></i>
        <i class="bi bi-search text-info fs-4" title="Consultar Formacion" id="btnConsultar"></i>
        <i
        ...
        ></i>
        <i class="bi bi-x-circle text-danger fs-4" title="Cancelar Formacion" id="btnCancelar"></i>

        <i class="bi bi-door-closed text-dark fs-4" title="Cerrar" id="btnCerrar"></i>

        <h4 class="fw-bold text-primary mb-0 ms-2">
          <i class="bi bi-file-earmark-text me-2"></i>Gestion de Formaciones
        </h4>
      </div>
      ...
      <div class="d-flex align-items-center gap-2">
        ...
        <div class="input-group" style="max-width:280px;">
          <input type="text" id="buscar" class="form-control form-control-sm" placeholder="Buscar formacion...">
      ...
      <div class="table-responsive" style="max-height:500px;overflow-y:auto;">
        <table class="table table-hover table-sm align-middle" id="tablaSolicitudes">
          ...
          <?php if ($solicitudes): foreach ($solicitudes as $row): ?>
              <tr data-id="<?= (int)$row['id'] ?>" data-nsolicitud="<?= (int)$row['nsolicitud'] ?>" data-estado="<?= esc($row['estado']) ?>">
              <td><?= esc($row['id']) ?></td>
...
(() => {
  ...
  tbody.addEventListener('click', (e) => {
    ...
    selectedEstado = tr.dataset.estado; // 👈 CLAVE
  });
  ...
  document.getElementById('btnNuevaSolicitud').onclick = () => {
    window.location.href = 'nueva_formacion.php';
  };
...
document.getElementById('btnCancelar').onclick = () => {
  ...
  if (rolContratista && selectedEstado !== 'I') {
    alert('⛔ Solo puedes cancelar solicitudes en estado Inicial.');
    return;
  }
  ...
  function abrirDetalle(nsol){
    if (!nsol) return;
    window.location.href = 'formacion_detalle.php?id=' + encodeURIComponent(nsol);
  }
  function abrirFinaliza(nsol){
    if (!nsol) return;
    window.location.href = 'finalizar_formacion.php?id=' + encodeURIComponent(nsol);
  }
    function abrirCancelar(nsol){
    if (!nsol) return;
    window.location.href = 'cancelar_formacion.php?id=' + encodeURIComponent(nsol);
  }
})();
</script>
</body>
