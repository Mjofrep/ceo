...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  $accion      = $_POST['accion'] ?? '';
  $id          = (int)($_POST['id'] ?? 0);
  $subgerencia = trim($_POST['subgerencia'] ?? '');
  $sufijo      = trim($_POST['sufijo'] ?? '');
  $estado      = trim($_POST['estado'] ?? 'A');

  if ($accion === 'crear' && $subgerencia) {
    $stmt = $pdo->prepare("INSERT INTO ceo_subgerencia (subgerencia, sufijo, estado)
                           VALUES (:subgerencia, :sufijo, :estado)");
    $stmt->execute(compact('subgerencia','sufijo','estado'));
    $msg = "✅ Subgerencia creada correctamente.";

  } elseif ($accion === 'editar' && $id > 0) {
    $stmt = $pdo->prepare("UPDATE ceo_subgerencia 
                           SET subgerencia=:subgerencia, sufijo=:sufijo, estado=:estado
                           WHERE id=:id");
    $stmt->execute(compact('subgerencia','sufijo','estado','id'));
    $msg = "📝 Subgerencia actualizada.";

  } elseif ($accion === 'eliminar' && $id > 0) {
    $pdo->prepare("DELETE FROM ceo_subgerencia WHERE id=?")->execute([$id]);
    $msg = "🗑️ Subgerencia eliminada.";
  }
}
...
/* ============================================================
   CARGA DE REGISTROS
   ============================================================ */
$subgerencias = $pdo->query("SELECT * FROM ceo_subgerencia ORDER BY id ASC")->fetchAll(PDO::FETCH_ASSOC);
?>
<!doctype html>
<html lang="es">
<head>
  <meta charset="utf-8">
  <title><?= APP_NAME ?> | Mantenimiento de Subgerencias</title>
...
<main class="container">
  ...
  <div class="card p-4 mb-4">
    <h4 class="mb-3">Agregar / Editar Subgerencia</h4>
    <form method="post" id="frmSubgerencia" class="row g-3">
      <input type="hidden" name="id" id="id">

      <div class="col-md-5">
        <label class="form-label">Nombre Subgerencia</label>
        <input type="text" class="form-control" name="subgerencia" id="subgerencia" required>
      </div>

      <div class="col-md-3">
        <label class="form-label">Sufijo</label>
        <input type="text" class="form-control" name="sufijo" id="sufijo" maxlength="10" placeholder="Ej: SG01">
      </div>
  ...
  <div class="card p-4">
    <h4 class="mb-3">Subgerencias Registradas</h4>
    <div class="table-responsive">
      <table class="table table-striped align-middle">
        ...
        <tbody>
        <?php foreach ($subgerencias as $s): ?>
          <tr>
            <td><?= $s['id']; ?></td>
            <td><?= $s['subgerencia']; ?></td>
            <td><?= $s['sufijo']; ?></td>
            <td><?= ($s['estado'] === 'A') ? 'Activo' : 'Desactivo' ?></td>
            <td>
              <button class="btn btn-sm btn-info btnEditar"
                      data-id="<?= $s['id']; ?>"
                      data-subgerencia="<?= $s['subgerencia']; ?>"
                      data-sufijo="<?= $s['sufijo']; ?>"
              ...
              <form method="post" class="d-inline">
                ...
                <button name="accion" value="eliminar" class="btn btn-sm btn-danger"
                        onclick="return confirm('¿Eliminar esta subgerencia?')">
                  Eliminar
                </button>
              </form>
            </td>
...
document.querySelectorAll('.btnEditar').forEach(btn => {
  btn.addEventListener('click', e => {
    ...
    document.getElementById('sufijo').value = d.sufijo;
...
document.getElementById('btnCancelar').addEventListener('click', () => {
  document.getElementById('frmSubgerencia').reset();
