<?php
// /public/mant_porcentaje_agrup_terreno.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'crear' && $id_agrupacion > 0 && $fechadesde) {
        ...
        $stmt = $pdo->prepare("
            INSERT INTO ceo_porcentaje_agrup_terreno
        ...
        $msg = "✅ Porcentaje de terreno creado correctamente.";

    /* ===================== EDITAR ===================== */
    } elseif ($accion === 'editar' && $id > 0) {

        $stmt = $pdo->prepare("
            UPDATE ceo_porcentaje_agrup_terreno
        ...
        $msg = "📝 Porcentaje de terreno actualizado.";

    /* ===================== ELIMINAR ===================== */
    } elseif ($accion === 'eliminar' && $id > 0) {

        $stmt = $pdo->prepare("DELETE FROM ceo_porcentaje_agrup_terreno WHERE id = :id");
        $stmt->execute([':id' => $id]);

        $msg = "🗑️ Porcentaje de terreno eliminado.";
    }
}
...
// Listado porcentajes terreno
$porcentajes = $pdo->query("
    SELECT p.*, a.grupo
    FROM ceo_porcentaje_agrup_terreno p
    INNER JOIN ceo_agrupacion_terreno a ON a.id = p.id_agrupacion
    ORDER BY a.grupo, p.fechadesde DESC
")->fetchAll(PDO::FETCH_ASSOC);
?>
<!doctype html>
<html lang="es">
<head>
<meta charset="utf-8">
<title><?= APP_NAME ?> | Mantenimiento Porcentaje Agrupación Terreno</title>
...
<div class="card p-4">
<h4 class="mb-3">Porcentajes Terreno Registrados</h4>
