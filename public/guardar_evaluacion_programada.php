<?php
declare(strict_types=1);
if (session_status() !== PHP_SESSION_ACTIVE) session_start();

header('Content-Type: application/json; charset=utf-8');

require_once '../config/db.php';
require_once '../config/functions.php';

function guardarHasColumn(PDO $pdo, string $table, string $column): bool
{
    static $cache = [];
    $key = $table . '.' . $column;
    if (array_key_exists($key, $cache)) {
        return $cache[$key];
    }

    $stmt = $pdo->prepare("
        SELECT 1
        FROM information_schema.COLUMNS
        WHERE TABLE_SCHEMA = DATABASE()
          AND TABLE_NAME = :table
          AND COLUMN_NAME = :column
        LIMIT 1
    ");
    $stmt->execute([
        ':table' => $table,
        ':column' => $column,
    ]);

    $cache[$key] = (bool)$stmt->fetch(PDO::FETCH_ASSOC);
    return $cache[$key];
}

function guardarFindProgramacion(PDO $pdo, string $rut, int $servicio, string $tipo, int $cuadrilla, ?int $idAgrupacion, bool $hasAgrupacionColumn): ?array
{
    $whereAgrupacion = '';
    $params = [
        ':rut' => $rut,
        ':servicio' => $servicio,
        ':tipo' => $tipo,
        ':cuadrilla' => $cuadrilla,
    ];

    if ($hasAgrupacionColumn && $idAgrupacion !== null) {
        $whereAgrupacion = ' AND id_agrupacion = :id_agrupacion';
        $params[':id_agrupacion'] = $idAgrupacion;
    }

    $stmtExiste = $pdo->prepare(" 
        SELECT id, estado, resultado
        FROM ceo_evaluaciones_programadas
        WHERE rut = :rut
          AND id_servicio = :servicio
          AND tipo = :tipo
          AND cuadrilla = :cuadrilla
          {$whereAgrupacion}
        ORDER BY id DESC
        LIMIT 1
    ");
    $stmtExiste->execute($params);
    $registro = $stmtExiste->fetch(PDO::FETCH_ASSOC);

    return $registro ?: null;
}

function guardarUpsertProgramacion(PDO $pdo, int $userId, string $rut, int $servicio, string $tipo, int $cuadrilla, int $idProcesoHab, ?int $idAgrupacion, ?array $registro, bool $hasAgrupacionColumn): void
{
    $idAgrupacionPersist = $hasAgrupacionColumn ? ($idAgrupacion ?? 0) : null;

    if ($registro) {
        if ($hasAgrupacionColumn) {
            $stmtUpd = $pdo->prepare(" 
                UPDATE ceo_evaluaciones_programadas
                SET estado = 'PENDIENTE',
                    resultado = 'PENDIENTE',
                    fecha_resultado = NULL,
                    usuario_programa = :usuario,
                    fecha_programacion = NOW(),
                    id_proceso_habilitacion = :id_proceso_habilitacion,
                    id_agrupacion = :id_agrupacion,
                    cobrado = 0
                WHERE id = :id
                LIMIT 1
            ");
            $stmtUpd->execute([
                ':usuario' => ($userId > 0 ? $userId : 1),
                ':id_proceso_habilitacion' => $idProcesoHab,
                ':id_agrupacion' => $idAgrupacionPersist,
                ':id' => (int)$registro['id']
            ]);
        } else {
            $stmtUpd = $pdo->prepare(" 
                UPDATE ceo_evaluaciones_programadas
                SET estado = 'PENDIENTE',
                    resultado = 'PENDIENTE',
                    fecha_resultado = NULL,
                    usuario_programa = :usuario,
                    fecha_programacion = NOW(),
                    id_proceso_habilitacion = :id_proceso_habilitacion,
                    cobrado = 0
                WHERE id = :id
                LIMIT 1
            ");
            $stmtUpd->execute([
                ':usuario' => ($userId > 0 ? $userId : 1),
                ':id_proceso_habilitacion' => $idProcesoHab,
                ':id' => (int)$registro['id']
            ]);
        }

        return;
    }

    if ($hasAgrupacionColumn) {
        $stmtIns = $pdo->prepare(" 
            INSERT INTO ceo_evaluaciones_programadas
                (rut, id_servicio, id_agrupacion, tipo, cuadrilla, id_proceso_habilitacion, fecha_programacion, usuario_programa, estado, intento, resultado, fecha_resultado, cobrado)
            VALUES
                (:rut, :servicio, :id_agrupacion, :tipo, :cuadrilla, :id_proceso_habilitacion, NOW(), :usuario, 'PENDIENTE', 1, 'PENDIENTE', NULL, 0)
        ");
        $stmtIns->execute([
            ':rut' => $rut,
            ':servicio' => $servicio,
            ':id_agrupacion' => $idAgrupacionPersist,
            ':tipo' => $tipo,
            ':cuadrilla' => $cuadrilla,
            ':id_proceso_habilitacion' => $idProcesoHab,
            ':usuario' => ($userId > 0 ? $userId : 1)
        ]);
        return;
    }

    $stmtIns = $pdo->prepare(" 
        INSERT INTO ceo_evaluaciones_programadas
            (rut, id_servicio, tipo, cuadrilla, id_proceso_habilitacion, fecha_programacion, usuario_programa, estado, intento, resultado, fecha_resultado, cobrado)
        VALUES
            (:rut, :servicio, :tipo, :cuadrilla, :id_proceso_habilitacion, NOW(), :usuario, 'PENDIENTE', 1, 'PENDIENTE', NULL, 0)
    ");
    $stmtIns->execute([
        ':rut' => $rut,
        ':servicio' => $servicio,
        ':tipo' => $tipo,
        ':cuadrilla' => $cuadrilla,
        ':id_proceso_habilitacion' => $idProcesoHab,
        ':usuario' => ($userId > 0 ? $userId : 1)
    ]);
}

if (empty($_SESSION['auth'])) {
    http_response_code(401);
    echo json_encode(['ok' => false, 'msg' => 'No autorizado']);
    exit;
}

$data = json_decode((string)file_get_contents("php://input"), true);

$rut       = trim((string)($data['rut'] ?? ''));
$servicio  = (int)($data['servicio'] ?? 0);
$cuadrilla = (int)($data['cuadrilla'] ?? 0);
$tipo      = (string)($data['tipo'] ?? '');
$checked   = (int)($data['checked'] ?? 0);

if ($rut === '' || $servicio <= 0 || $cuadrilla <= 0 || !in_array($tipo, ['PRUEBA','TERRENO'], true)) {
    http_response_code(400);
    echo json_encode(['ok' => false, 'msg' => 'Datos inválidos']);
    exit;
}

try {
    $pdo = db();
    $userId = (int)($_SESSION['auth']['id'] ?? 0);
    $idAgrupacion = (int)($data['id_agrupacion'] ?? 0);
    $hasEvaluacionProgramadaAgrupacion = guardarHasColumn($pdo, 'ceo_evaluaciones_programadas', 'id_agrupacion');

    $registro = guardarFindProgramacion($pdo, $rut, $servicio, $tipo, $cuadrilla, null, $hasEvaluacionProgramadaAgrupacion);

    $lleeConfig = ceoIsLleeService($servicio) ? ceoGetLleeConfig() : null;

    if ($tipo === 'PRUEBA') {
        if ($checked && $idAgrupacion <= 0 && $lleeConfig === null) {
            throw new Exception('Debe seleccionar la prueba a aplicar antes de programarla.');
        }

        if ($checked && $idAgrupacion > 0 && !$hasEvaluacionProgramadaAgrupacion) {
            throw new Exception('Falta la columna id_agrupacion en ceo_evaluaciones_programadas para guardar la prueba seleccionada.');
        }

        if ($idAgrupacion > 0) {
            $stmtAgrupacion = $pdo->prepare("
                SELECT id
                FROM ceo_agrupacion
                WHERE id = :id
                  AND id_servicio = :id_servicio
                LIMIT 1
            ");
            $stmtAgrupacion->execute([
                ':id' => $idAgrupacion,
                ':id_servicio' => $servicio
            ]);

            if (!$stmtAgrupacion->fetch(PDO::FETCH_ASSOC)) {
                throw new Exception('La prueba seleccionada no pertenece al servicio indicado.');
            }
        }
    } else {
        $idAgrupacion = 0;
    }

    if ($checked) {
        $pdo->beginTransaction();

        $idCargo = obtenerCargoTrabajador($pdo, $rut, $servicio, $cuadrilla);
        if ($idCargo === null) {
            throw new Exception('No se pudo determinar el cargo del trabajador para este servicio.');
        }

        $procesoHab = resolverProcesoHabilitacionParaProgramacion($pdo, $rut, $servicio, $idCargo);
        if ($procesoHab === null) {
            throw new Exception('Debe generar o seleccionar un proceso abierto desde el detalle del trabajador antes de programar la evaluación.');
        }
        $idProcesoHab = (int)$procesoHab['id'];

        if ($lleeConfig !== null && $tipo === 'PRUEBA') {
            if (!$hasEvaluacionProgramadaAgrupacion) {
                throw new Exception('Falta la columna id_agrupacion en ceo_evaluaciones_programadas para programar LLEE y Hotline.');
            }

            foreach ([(int)$lleeConfig['theory_group_id'], (int)$lleeConfig['hotline_group_id']] as $agrupacionLlee) {
                $registroLlee = guardarFindProgramacion($pdo, $rut, $servicio, $tipo, $cuadrilla, $agrupacionLlee, true);
                guardarUpsertProgramacion($pdo, $userId, $rut, $servicio, $tipo, $cuadrilla, $idProcesoHab, $agrupacionLlee, $registroLlee, true);
            }

            $pdo->commit();

            echo json_encode(['ok' => true, 'msg' => 'Programacion LLEE y Hotline creada']);
            exit;
        }

        guardarUpsertProgramacion(
            $pdo,
            $userId,
            $rut,
            $servicio,
            $tipo,
            $cuadrilla,
            $idProcesoHab,
            $tipo === 'PRUEBA' && $idAgrupacion > 0 ? $idAgrupacion : null,
            $registro,
            $hasEvaluacionProgramadaAgrupacion
        );

        $pdo->commit();

        echo json_encode(['ok' => true, 'msg' => 'Programación creada']);
        exit;
    }

    // Desmarcar: si existe, dejar ANULADA
    if ($lleeConfig !== null && $tipo === 'PRUEBA' && $hasEvaluacionProgramadaAgrupacion) {
        $pdo->beginTransaction();

        $stmtUpdLlee = $pdo->prepare(" 
            UPDATE ceo_evaluaciones_programadas
            SET estado = 'ANULADA'
            WHERE rut = :rut
              AND id_servicio = :servicio
              AND tipo = 'PRUEBA'
              AND cuadrilla = :cuadrilla
              AND id_agrupacion IN (:agrup_teoria, :agrup_hotline)
        ");
        $stmtUpdLlee->execute([
            ':rut' => $rut,
            ':servicio' => $servicio,
            ':cuadrilla' => $cuadrilla,
            ':agrup_teoria' => (int)$lleeConfig['theory_group_id'],
            ':agrup_hotline' => (int)$lleeConfig['hotline_group_id'],
        ]);

        $pdo->commit();

        echo json_encode(['ok' => true, 'msg' => 'Programacion LLEE y Hotline anulada']);
        exit;
    }

    if ($registro) {
        $pdo->beginTransaction();

        $stmtUpd = $pdo->prepare("
            UPDATE ceo_evaluaciones_programadas
            SET estado = 'ANULADA'
            WHERE id = :id
            LIMIT 1
        ");
        $stmtUpd->execute([
            ':id' => (int)$registro['id']
        ]);

        $pdo->commit();
    }

    echo json_encode(['ok' => true, 'msg' => 'Programación anulada']);

} catch (Throwable $e) {
    if (isset($pdo) && $pdo instanceof PDO && $pdo->inTransaction()) {
        $pdo->rollBack();
    }
    http_response_code(500);
    echo json_encode(['ok' => false, 'msg' => 'Error SQL: ' . $e->getMessage()]);
}
