...
<head>
<meta charset="utf-8">
<title>Formaciones - Finalizar Formacion #<?= htmlspecialchars((string)($sol['nsolicitud'] ?? '—')) ?> - <?= htmlspecialchars((string)APP_NAME) ?></title>
...
<div class="container mb-5">
  <h5 class="text-primary mb-3">
    ...
    Finalizar Formacion N° <?= htmlspecialchars((string)($sol['nsolicitud'] ?? '')) ?>
  </h5>
  ...
  <div class="card rounded-4 mb-4">
    <div class="card-body">
      ...
      <div class="text-end mt-4">
        <button id="btnFinalizar" class="btn btn-success btn-sm px-4">
          <i class="bi bi-save me-2"></i>Grabar Finalización
        </button>
      </div>
    </div>
  </div>
...
document.getElementById('btnFinalizar').addEventListener('click', async ()=>{
  if(!confirm('¿Desea marcar esta solicitud como Finalizada?')) return;
  const fd = new FormData();
  fd.append('id_solicitud', <?= (int)$sol['nsolicitud'] ?>);
  try{
    const resp = await fetch('finalizar_formacion_update.php',{method:'POST',body:fd});
    const data = await resp.json();
    if(data.ok){
      alert('✅ Formacion finalizada correctamente.');
      window.location.href = 'formaciones.php';
    } else {
      alert('⚠️ Error al finalizar: '+(data.error||''));
    }
  }catch(err){
