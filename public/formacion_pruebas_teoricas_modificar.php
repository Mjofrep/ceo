<?php
// --------------------------------------------------------------
// formacion_pruebas_teoricas_modificar.php - CEO
...
if (!empty($_POST['nueva_alt_texto'])) {
  foreach ($_POST['nueva_alt_texto'] as $idx => $nuevoTexto) {
    ...
    if ($action === 'delete_pregunta') {
      ...
      $pdo->prepare("DELETE FROM ceo_formacion_alternativas_preguntas WHERE id_pregunta=?")->execute([$id]);
      $pdo->prepare("DELETE FROM ceo_formacion_preguntas_servicios WHERE id=?")->execute([$id]);
    ...
    if ($action === 'delete_alternativa') {
      ...
      $pdo->prepare("DELETE FROM ceo_formacion_alternativas_preguntas WHERE id=?")->execute([$id]);
