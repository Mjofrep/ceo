...
if ((int)($_SESSION['auth']['id_rol'] ?? 0) === 6) {
  echo "<div class='alert alert-danger m-5'>No autorizado para cancelar solicitudes.</div>";
  exit;
}
...
<head>
<meta charset="utf-8">
<title>Cancelar Solicitud #<?= htmlspecialchars((string)($sol['nsolicitud'] ?? '—')) ?> - <?= htmlspecialchars((string)APP_NAME) ?></title>
...
<div class="container mb-5">
  <h5 class="text-danger mb-3">
    ...
    Cancelar Solicitud N° <?= htmlspecialchars((string)($sol['nsolicitud'] ?? '')) ?>
  </h5>
    ...
    <div class="card-body">
      ...
      <div class="text-end mt-4">
        <button id="btnCancelar" class="btn btn-danger btn-sm px-4">
          <i class="bi bi-save me-2"></i>Cancelar Solicitud
        </button>
      </div>
    </div>
  </div>
...
document.getElementById('btnCancelar').addEventListener('click', async ()=>{
  ...
  try{
    const resp = await fetch('cancelar_solicitud_update.php',{method:'POST',body:fd});
