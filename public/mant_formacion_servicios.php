<?php
// /public/mant_formacion_servicios.php
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($accion === 'crear' && $servicio !== '') {
        $stmt = $pdo->prepare("INSERT INTO ceo_formacion_servicios (servicio, descripcion)
                               VALUES (:servicio, :descripcion)");
        $stmt->execute([
            ':servicio' => $servicio,
            ':descripcion' => $descripcion,
        ]);
        $msg = 'Servicio de formación ingresado correctamente.';

    } elseif ($accion === 'editar' && $id > 0 && $servicio !== '') {
        $stmt = $pdo->prepare("UPDATE ceo_formacion_servicios
        ...
        $stmt = $pdo->prepare('DELETE FROM ceo_formacion_servicios WHERE id = :id LIMIT 1');
        $stmt->execute([':id' => $id]);
...
/* ============================================================
   CARGA DATOS
============================================================ */
$rows = $pdo->query('SELECT id, servicio, descripcion FROM ceo_formacion_servicios ORDER BY id DESC')->fetchAll(PDO::FETCH_ASSOC);
?>
<!doctype html>
<html lang="es">
<head>
<meta charset="utf-8">
<title><?= APP_NAME ?> | Servicios de Formación</title>
...
.dataTables_filter input { border-radius: 0.5rem !important; padding: .4rem .6rem !important; }
.dataTables_length select { border-radius: 0.5rem; }
.dataTables_wrapper .dataTables_paginate .paginate_button { padding: .3rem .7rem; margin: 2px; }
...
<div class="card p-4 mb-4">
  <h4 class="mb-3">Agregar / Editar Servicio de Formación</h4>

  <form method="post" id="frmFormacionServicio" class="row g-3">
...
<div class="card p-4">
  <h4 class="mb-3">Servicios de Formación</h4>

  <div class="table-responsive">
    <table class="table table-striped align-middle" id="tablaFormacionServicios">
        ...
        <tr>
          ...
          <td>
            <button class="btn btn-info btn-sm btnEditar"
                    data-row='<?= esc(json_encode($r, JSON_UNESCAPED_UNICODE)) ?>'>
              Editar
            </button>
            ...
            <form method="post" class="d-inline">
              ...
              <button name="accion" value="eliminar" class="btn btn-danger btn-sm"
                      onclick="return confirm('¿Eliminar este servicio de formación?')">
                Eliminar
              </button>
            </form>
          </td>
...
$(document).ready(function(){
    var table = $('#tablaFormacionServicios').DataTable({
        orderCellsTop: true,
        fixedHeader: true,
        pageLength: 25,
    ...
    $('#tablaFormacionServicios thead tr:eq(1) th input').on('keyup change', function(){
...
document.getElementById('btnCancelar').addEventListener('click', () => {
    document.getElementById('frmFormacionServicio').reset();
