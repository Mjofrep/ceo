<?php
// --------------------------------------------------------------
// solicitud_autoriza_envio.php - Envío de Autorización (PDF adjunto)
// --------------------------------------------------------------
...
if ($idSolicitud <= 0) {
  echo "<div class='alert alert-danger m-5'>Solicitud inválida.</div>";
  exit;
}
...
if (!$S) {
  echo "<div class='alert alert-warning m-5'>No se encontró la solicitud N° {$idSolicitud}.</div>";
  exit;
}
...
SELECT ps.rut, ps.nombre, ps.apellidop, ps.apellidom,
    ...
    FROM ceo_participantes_solicitud ps, ceo_solicitudes so, ceo_empresas em
...
<div class="header">
  <div>
    <h2>Autorizada la Solicitud  N° <?= htmlspecialchars((string)$idSolicitud) ?></h2>
...
<table class="tbl meta">
  <tr>
    ...
    <td><b>Capacitación</b> <span class="box"><?= htmlspecialchars($charla) ?></span></td>
  </tr>
  <tr>
...
// Nombre y ruta final
$nombreArchivo = "Permiso_Solicitud_{$idSolicitud}.pdf";
...
try {
  ...
  // Destinatarios solicitados
  $destinatarios = [
      'leonel.morales2.external@enel.com',
      'yanett.henriquez@enel.com',
      'marcelo.jofre.external@enel.com'
  ];
  ...
  $mail->Subject  = "Autorización para Ingresar al CEO Solicitud N° {$idSolicitud}";
...
try {
  ...
  $mail2->Subject  = "Listado de Participantes - Solicitud N° {$idSolicitud} ({$fecha})";
...
$stDoc->execute([
  ...
  $mail2->Body = "
    <p>Estimados,</p>
    <p>Se informa el proceso de habilitación correspondiente al <b>{$fecha}</b> para la <b>Solicitud N° {$idSolicitud}</b>.</p>
    <p>Participantes:</p>
    {$tabla}
    <p>Se adjunta el documento PDF de autorización correspondiente.</p>
    <br><p>Saludos,<br>Sistema CEO</p>
  ";
  $mail2->AltBody = "Listado de participantes para la solicitud N° {$idSolicitud} ({$fecha})";
...
$mail2->addAttachment($rutaFinal, $nombreArchivo);
  ...
  error_log("⚠️ Error al enviar correo a Marcelo Jofré (Solicitud {$idSolicitud}): {$mail2->ErrorInfo}");
}

  // Limpieza del archivo temporal
  //@unlink($tmpName);
  header("Location: solicitudes.php?msg=autorizacion_ok");
  //header("Location: solicitud_detalle.php?id={$idSolicitud}&msg=autorizacion_ok");
  exit;

} catch (Exception $e) {
  error_log("❌ Error al enviar autorización N° {$idSolicitud}: {$mail->ErrorInfo}");
  //@unlink($tmpName);
  header("Location: solicitud_detalle.php?id={$idSolicitud}&msg=autorizacion_error");
  exit;
}
