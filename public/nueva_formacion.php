<?php
// --------------------------------------------------------------
// nueva_formacion.php - Centro de Excelencia Operacional (CEO)
...
if (isset($_POST['action']) && $_POST['action'] === 'guardar_solicitud') {
  ...
  try {
    ...
    $next = (int)$pdo->query("SELECT COALESCE(MAX(nsolicitud),0)+1 FROM ceo_formacion_solicitudes")->fetchColumn();
if ($next === 5030) {
    $next = 5528;
}
    ...
    $stmt = $pdo->prepare("
    INSERT INTO ceo_formacion_solicitudes
    ...
    if ($fecha && $patio && $hIni && $hFin) {
      try {
        $upd = $pdo->prepare("
          UPDATE ceo_formacion_calendario
             SET estado = 'OCUPADO',
...
if (!empty($rows)) {
  ...
  $partStmt = $pdo->prepare("
    INSERT INTO ceo_formacion_participantes_solicitud
  ...
  $updWf = $pdo->prepare("
    UPDATE ceo_formacion_participantes_solicitud
       SET wf = :wf
...
$habCeo         = fetchPairs($pdo,"SELECT id,desc_tipo AS nombre FROM ceo_formaciontipo ORDER BY desc_tipo");
...
/*  $sql = "SELECT 
            ...
            p.desc_patios
          FROM ceo_formacion_calendario c
          LEFT JOIN ceo_formacion_solicitudes s ON s.nsolicitud = c.nsolicitud
...
FROM `ceo_formacion_solicitudes` a
...
<head>
<meta charset="utf-8">
<title>Formaciones - Nueva Formacion - <?=esc(APP_NAME)?></title>
  ...
  <div class="row g-4 mb-4">
    ...
    <div class="col-lg-6">
      <div class="card h-100 rounded-4">
        <div class="card-body">
          <h6 class="fw-bold mb-3 text-primary">Formulario de Nueva Formacion</h6>
...
<input type="file" id="inputExcel" accept=".xlsx,.xls,.csv" style="display:none;">
  ...
  <div class="text-end mb-5">
    <button id="btnGuardar" class="btn btn-primary"><i class="bi bi-save me-1"></i> Guardar Formacion</button>
  </div>
</div>
...
async function validarFechaBloqueada(el) {
  ...
  try {
    const r = await fetch(`nueva_formacion.php?action=validar_fecha_bloqueo&fecha=${encodeURIComponent(fecha)}`);
...
if (selUO && selSrv) {
  selUO.addEventListener('change', async function () {
    ...
    try {
      const r = await fetch(`nueva_formacion.php?action=servicios_by_uo&uo_id=${encodeURIComponent(uoId)}`);
      const data = await r.json();
...
if (selSrv && selRespUO) {
  selSrv.addEventListener('change', async function () {
    ...
    try {
      const resp = await fetch(`nueva_formacion.php?action=responsable_by_servicio&servicio_id=${encodeURIComponent(servicioId)}`);
...
document.getElementById('btnGuardar').onclick = async () => {
  ...
  try {
    const resp = await fetch('nueva_formacion.php', {
      method: 'POST',
      body: fd
    });
    ...
    if (data.ok) {
      alert('✅ Formacion guardada N° ' + data.nsolicitud);
      window.location.href = 'enviar_correo_formacion.php?nsolicitud=' + data.nsolicitud;
