...
SELECT
    ...
    h.nsolicitud
FROM ceo_formacion h
INNER JOIN ceo_empresas ce ON ce.id = h.empresa
INNER JOIN ceo_formacion_servicios sp ON sp.id = h.id_servicio
...
<head>
<meta charset="utf-8">
<title>Formaciones - Planificacion - <?= esc(APP_NAME) ?></title>
...
<div class="card shadow-sm mb-3">
  <div class="card-body d-flex justify-content-between align-items-center">
    <h5 class="fw-bold text-primary mb-0">
      <i class="bi bi-clipboard-check me-2"></i>Registros de Formaciones
    </h5>
...
Agradecere realizar el proceso de Formacion. DD/mm/yyyy
  ...
  document.querySelectorAll('tbody tr').forEach(tr => {
    tr.addEventListener('dblclick', () => {
      ...
      window.location.href =
        `revision_formaciones.php?cuadrilla=${cuadrilla}&empresa=${empresa}&uo=${encodeURIComponent(uo)}`;
    });
  });
...
function formatFecha(fechaISO) {
  ...
  document.getElementById('btnOrden').addEventListener('click', () => {
    ...
    document.querySelector('input[name="asunto"]').value =
      `Formacion ${fechaFormateada}`;
    
    // 👉 Reemplazar cuerpo del correo
    document.querySelector('textarea[name="cuerpo"]').value =
    `Estimados,
    
    Agradecere realizar el proceso de Formacion con fecha ${fechaFormateada}.
    
    Saludos cordiales.`;

    const cuadrillas = filas.map(tr => tr.dataset.cuadrilla);

    fetch('obtener_participantes_formacion.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    .catch(() => {
      alert('Error al obtener participantes. Revisa obtener_participantes_formacion.php');
    });
...
document.getElementById('formOrden').addEventListener('submit', function (e) {
  ...
  fetch('/ceo.noetica.cl/public/enviar_orden_mail_formacion.php', {
    method: 'POST',
    body: formData
  })
  .then(r => {
